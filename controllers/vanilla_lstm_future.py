...
class Controller(BaseController):
    ...
    def __init__(
        self,
        model_path="../models/v4_lstm-2-256_10000-dataset_lr-0.001_epochs-20_seq-100_local-scaler_futurek-20_target-steer-filtered/lstm_future_best.pt",
        scaler_path="../models/v4_lstm-2-256_10000-dataset_lr-0.001_epochs-20_seq-100_local-scaler_futurek-20_target-steer-filtered/scalers.pkl",
        seq_len: int = 100,
        future_k: int = 20,
        hidden_size: int = 256,
        num_layers: int = 2,
        ...
        state_dict = torch.load(model_abs_path, map_location=self.device)
        missing, unexpected = self.model.load_state_dict(state_dict, strict=False)
        if missing:
            print(f"[vanilla_lstm_future] Warning: missing keys in state_dict: {missing}")
        if unexpected:
            print(f"[vanilla_lstm_future] Warning: unexpected keys in state_dict: {unexpected}")
        self.model.eval()
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        ...
        if len(self.physics_history) < self.seq_len:
            # Provide a smoother ramp by scaling proportional gain with available history fraction
            frac = len(self.physics_history) / self.seq_len
            error = (target_lataccel - current_lataccel)
            return 0.3 * frac * error
