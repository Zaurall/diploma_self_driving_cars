...
from train.reinforcement_learning.ppo.gym_env_v2 import PPOEnv
...
def collect_data_files(data_dir: str, limit: int = 5000):
    """Collect CSV data files for multi-file training."""
    data_dir = Path(data_dir)
    if data_dir.is_file():
        return [str(data_dir)]
    elif data_dir.is_dir():
        files = sorted(data_dir.glob("*.csv"))[:limit]
...
def make_custom_env(rank, model_path, data_files):
    def thunk():
        model = TinyPhysicsModel(model_path=model_path, debug=False)
        data_path = data_files[rank % len(data_files)]
        env = PPOEnv(simulation_model=model, data_path=data_path, debug=False)
        return env
    return thunk
...
class Agent(nn.Module):
    def __init__(self, n_obs, n_act, device=None):
        ...
        self.critic = nn.Sequential(
            ...
            nn.Tanh(),
            layer_init(nn.Linear(64, 32, device=device)),
            nn.Tanh(),
            layer_init(nn.Linear(32, 1, device=device), std=1.0),
        )
        self.actor_mean = nn.Sequential(
            ...
            layer_init(nn.Linear(64, 32, device=device)),
            nn.Tanh(),
            layer_init(nn.Linear(32, n_act, device=device), std=0.01),
        )
...
if __name__ == "__main__":
    ...
    ####### Environment setup #######
    model_path = r"./models/tinyphysics.onnx"
    data_dir = r"./data/test"

    data_files = collect_data_files(data_dir, limit=5000)

    envs = gym.vector.SyncVectorEnv([
        make_custom_env(i, model_path, data_files) for i in range(args.num_envs)
    ])
    ...
    for iteration in pbar:
        ...
        envs.envs = [make_custom_env(i, model_path, data_files)() for i in range(args.num_envs)]
