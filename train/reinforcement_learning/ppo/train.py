...
from train.reinforcement_learning.ppo.policy import TransformerPPOPolicy, FFPPOPolicy, FFComplexPPOPolicy
...
class PPOTrainer:
    ...
    def save_training_statistics(self):
        np.savez(
            f"{self.save_dir}/training_stats.npz",
            **{k: np.array(v, dtype=float) for k, v in self.policy_logs.items()}
        )

        with open(f"{self.save_dir}/training_stats.json", "w") as f:
            json.dump(self.policy_logs, f, indent=2)

    def plot_training_dynamics(self, save: bool = False):
        ...
        if save and self.save_dir is not None:
            out_path = f"{self.save_dir}/training_dynamics.png"
    ...
    def create_env(self, data_path: str) -> List[str]:
        ...
        if data_path.is_dir():
            ...
            files = sorted(data_path.glob('*.csv'))[:100]
    ...
    def train(self, num_rollouts=1000):
        ...
        for rollout_idx in pbar:
            ...
            if avg_reward > self.best_reward:
                ...
                best_path = os.path.join(self.save_dir, "best_model_weights.pth")
                torch.save(self.policy.state_dict(), best_path)
                print(f"New best model saved with avg_reward={avg_reward:.4f} at {best_path}")
            ...
            # Save checkpoint
            if rollout_idx % 4 == 0:
                torch.save(self.policy.state_dict(), f"{self.save_dir}/model_weights_rollout_{rollout_idx}.pth")
                try:
...
def main():
    ...
    # model_path = r'./models/ppo_transformer-small_20-rollouts'
    model_path = r'./models/ppo_ff_tanh_256-128-64_dropout-0_20-rollouts_20-dataset_normalized'
    ...
    trainer.plot_training_dynamics(save=True)
    trainer.save_training_statistics()
